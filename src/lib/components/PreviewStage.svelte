<script lang="ts">
    ...
    interface Props {
        ...
        customPos: { horizontal: string, vertical: string, x: number, y: number };
        logoScale: number;
    }

    let { imageFile, logoFile, mode, position = $bindable(), customPos, logoScale }: Props = $props();
    ...
    let logoStyles = $derived.by(() => {
        ...
        if (mode === 'custom') {
            const hl = customPos.horizontal === 'left' ? `${customPos.x}px` : 'auto';
            const hr = customPos.horizontal === 'right' ? `${customPos.x}px` : 'auto';
            const vt = customPos.vertical === 'top' ? `${customPos.y}px` : 'auto';
            const vb = customPos.vertical === 'bottom' ? `${customPos.y}px` : 'auto';
...
>
    {#if imageSrc}
        <img class="base-image" src={imageSrc} alt="Preview" draggable="false" />
    {:else}
